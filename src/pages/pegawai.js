...
class Pegawai extends React.Component {
    ...
    getPegawai = () => {
        let url = "http://localhost:2021/pegawai";
    ...
    handleSave = (event) => {
        ...
        if (this.state.action === "insert") {
          url = "http://localhost:2021/pegawai/save"
        } else {
          url = "http://localhost:2021/pegawai/update"
        }
    ...
    Drop = (id_pegawai) => {
        let url = "http://localhost:2021/pegawai/" + id_pegawai;
        // memanggil url API untuk menghapus data pada database
        if (window.confirm('wahh, yakin diapus nih??')) {
          axios.delete(url)
          .then(response => {
